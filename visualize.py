...
if __name__ == '__main__':
    ...
    for bi, batch_dict in enumerate(test_dataloader):
        ...
        # save_open3d_render(points_xyz=np.ascontiguousarray(batch_dict['sp_features'][:, :, :3].reshape(-1, 3)), 
        #                    intensities=np.ascontiguousarray(batch_dict['sp_features'][:, :, -1].reshape(-1)), 
        #                    boxes_kitti=None, 
        #                    filename='rdr.png')
        
        # save_open3d_render(points_xyz=np.ascontiguousarray(batch_dict['voxels'][:, :, :3].reshape(-1, 3)), 
        #             intensities=np.ascontiguousarray(batch_dict['voxels'][:, :, -1].reshape(-1)), 
        #             boxes_kitti=None, 
        #             filename='ldr.png')
        ...
        points_xyz = attrs[:, :, :3].detach().cpu().numpy().reshape(-1, 3)
        intensity = attrs[:, :, -1].detach().cpu().numpy().reshape(-1)
        ...
        # save_open3d_render(points_xyz=points_xyz, intensities=intensity, boxes_kitti=None, filename='pred.png')
